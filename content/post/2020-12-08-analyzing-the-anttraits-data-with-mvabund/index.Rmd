...
Both, the fan-shape and the deviation from the 1:1 line in the Q-Q plot are gone. 
This means that the negative-binomial models fits the data better and will be used for the rest if this analysis. 
In the upper right corner of the Q-Q plot the observed values actually fall below the 1:1 line now which means that the residuals are underdispersed. 
Possibly this is because that data are rather zero-inflated and not overdispersed. 
Indeed, a zero-inflated-Poisson gllvm model fit the data better than a negative binomial one. In *<span style="color:CornflowerBlue">mvabund</span>* we currently cannot fit such models, so we will move ahead with the negative binomial one. 
...
The test statistic (here the deviance or likelihood ratio) and lastly the pseudo *p*-value. 
...
print(plot.tas)
```
The problem with this plot is, that it is scaled relative the actually occurring values and one value (Volume.lying.CWD for *Cardiocondyla nuda atalanta*) is strongly negative. 
To avoid this we can either use an absolute scale, base the relative scale on another static, remove the species or the variable. 
