...
```{r}
#TODO fill good places for copulas 
#TODO Available error rates 
```


In this blog post I will go though the copula model for ecological data that was proposed by @Anderson2019. 
I will not go into more details concerning copulas in general so you should have a basic familiarity with the topic. Good places to get acquainted with copulas are: 
...
Like the authors of the paper we will use the poor knight fish data which you can download as part of the supplementary materials of the [paper](https://onlinelibrary.wiley.com/doi/full/10.1002/ece3.4948). There you can also find the r scripts 1 to 3 (named ece34948-sup-0003-DataS1.txt, ece34948-sup-0004-DataS2.txt and ece34948-sup-0005-DataS3.txt). In the code below we load the data and the renamed R scripts. Note that the  original third code file also contains an example use case. I split the file into one containing the functions which we load here and another one which contains the example use case, which I don't load.
...
The "treatment" for in this example is the season at which the fish were observed. The data contain three different seasons:  September 1998 and 99 as well as March 99. We want to construct a different copula model for each season se we need to separate the data set. 
The select verb from dplyr is explicitly
...
Next we define a function to: 1. Subset data to a certain value of the variable
...
Don't repeat yourself. Which means that in coding you should avoid writing the
same lines of code over and over again to apply them to different objects.
This makes it more cumbersome to fix errors or to rerun the
analysis.
...
occasionally returns an error. If that happens to you, simply rerun it, you might even want to warp it in a while loop combined with purrr's safely to let the function run until it finishes without error. 

The arguments are the data set (Y), the number of permutations (9999), the type of correction applied to handle family-wise error rate (alpha_type), should a graph be ploted (graphic) and the significance level (sig_level). 
...
Now comes the part which was hardest for to figure out because it is not included in the provided code. If you compare all we have done with figure 6 in @Anderson2019 we are only at c. The next step included simulating new data sets from our copulas to create a new super matrix. This we can use to compute a super distance matrix from which we will derive centroids for each treatment (season).   
We are going to do 100 separate simulations (n_sim). The number of sites belonging to each season will be keept constant at the original levels which are stored in n_g1-3. dt_base is a template with will be used as a starting point in each simulation. In its original forms it only has a Sample column which gives each row an id and a Treatment (here: season) column which assigns a treatment level to each observation.
Each iteration of the for loop fills a copy of dt_base with the simulated abundances and saves its results as an element in the list sim_list. After the loop is finished we combine all single data sets into one super set (sim_mat). In the loop we also create null model simulations in which the observations are randomly shuffled after drawing them from the simulations.
This super set is quite large and computing its distance matrix can is computationally demanding. To speed this up we use the parrallelDist package which enables us to use multiple cores for this task. Lastly we can find the centroids of each season with the betadisper function form vegan. 
