...
class(germany_area)
```
Das Erbgeniss hat die Klasse `units` die das gleichnamig [Package](https://r-quantities.github.io/units/articles/measurement_units_in_R.html) eingeführt hat. 
Mit `units::drop_units()` können wir das Objekt in einfache Zahlen umwandeln ... 
```{r}
germany_area <- units::drop_units(germany_area)
```

... und die Verwaltungsbezirke ihrer Fläche nach auf einer Karte einfärben. 
...
`mapview` ist oft die schnellere und einfachere Lösung um interaktive Karten zu erstellen. 
`tmap` bietet mehr Optionen und ist die bessere Lösung um Karten für Berichte zu erstellen. 

Mit `st_distance` können wir die Distanz zwischen zwei Objekten ermitteln. 
Als Beispiel könnten wir uns Fragen ob die 10 größten Bezirke näher aneinander liegen als die zehn kleinsten. 
...
Die `st_distance()` kann mit einem oder zwei Datensätzen benutzt werden. In beidem Fällen ist das Ergebniss eine matrix mit units. In unserem Fall, also mit nur einem Datensatz, wird die Distanz von jeden Polygon zu jedem Polygon berechnet. In Zelle 3, Reihe 2 ist die Distanz von Zentroid des dritten Polyongs zum zweiten. Da die Distanz von dritten zum zweiten gleich der vom zweiten zum dritten ist, ist die Matrix symmetrisch. Die Diagoale enthält die Distanzen von Objekten zu sich selbst, also 0.   
...
Mit `upper.tri()` wähle ich das obere Dreick der Matrix aus (siehe Abbildung ) damit ich nich jeden Wert doppelt habe. Außerdem wird so die Diagonale entfernt. 
![Das obere Dreick einer Matrix](images/upper-triangular-matrix.png)


Für `small10` machen wir das gleiche, aber diesmal mit Pipe-Operatoren. Die einzigen Unterschiede sind hier, dass `slice_tail()` druch `slice_head()` ersetzt wurde und die annonyme Funktion am Ende der Pipe. Annonyme Funktionen sind Funktionen ohne Namen. Sie werden nich als Objekt gespeichert sonder direkt ausgeführt. In R können wir seit der Version 4.1 annonyme Funktionen wie folgt erstellen: 

```{r}
# Die beiden Funktionen machen das gleiche. 
plus5 <- function(x) return(x+5)
plus5(4)
4 |> {\(x) x + 5}()
```

Jetzt müssen wir die Ergebnisse noch in einem Datensatz verpacken und sie darstellen. Für zweiteres nutzen wir hier das `ggplot2` Package. Siehe [hier](https://stats.oarc.ucla.edu/stat/data/intro_ggplot2_int/ggplot2_intro_interactive_flat.html) für eine Einführung. 
...
So sehen wir, dass die Median Distanz zwischen den zehn größten Bezirken etwas kleiner ist als die zwischen den zehn kleinsten. 

Wenn wir zwei Datensätze haben wo wir immer nur das erste Element aus Datensatz 1 mit dem ersten Element aus Datensatz 2 vergleichen wollen, setzen wir in der Funktion `st_distance()` das Argument `by_element` auf `TRUE`. 
...
Dabei habe wir die Position der gewünschten Objekte in der Tabelle (bei `[`), ihren Spaltennamen (bei `select()`) oder Werte der verschiedenen Variablen (bei `filter()`) genutzt. 
Bei räumlichen Daten bietet sich nun auch räumliche Bezihungen an um Daten zu subsetten. 
Die Syntax folgt dabei folgenden Schema: Lasse `X` den Datensatz sein aus dem wir auswählen wollen und lasse `Y` den Datensatz sein anhand dessen wir auswählen. 
...
# meisen2 <- meisen |> select(gbifID, locality, occurrenceStatus, individualCount, decimalLatitude, decimalLongitude)
# saveRDS(meisen2, "meisen2.rds")
...
Alternativen sind berühren der Objekte (`st_touch()`), kreuzen der Objekte (`st_cross()`) und Überdecken (`st_covers()`). Siehe die folgende Abbildung für mehr Beispiele. 
