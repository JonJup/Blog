...
plot(mpost[[1]]$Beta)
```
```{r mcmc-plots2, fig.align="center"}
plot(mpost[[2]]$Beta)
```
```{r mcmc-plots3, fig.align="center"}
plot(mpost[[3]]$Beta)
```

Based on these plots we can see that using a thinning of 100 is necessary for satisfactory MCMC convergence. In addition to these visual checks we can also look at the potential scale reduction factor (aka Gelman statistic) and the effective samples size. 
```{r effective sample size show}
ess.beta = effectiveSize(mpost3$Beta)
```
```{r effective sample size dont show, echo=F}
